...
-- I changed this! adding manual modifiers to keys
function obj.singleKey(mod, key, name)
	-- local mod = {}
...
function obj.recursiveBind(keymap)
	...
	for key, map in pairs(keymap) do
		...
		for _, esc_k in ipairs(obj.escapeKeys) do
			modal:bind(esc_k[1], esc_k[2], function()
				modal:exit()
				killHelper()
			end)
		end
		-- modal:bind(obj.escapeKeys[1], obj.escapeKey[2], function() modal:exit() killHelper() end)
		if #key >= 3 then
