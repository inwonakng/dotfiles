...
Before claiming completion, use the `verify` skill and report only what fresh evidence proves. Report the active workspace, then call the `workspace` tool with `action=integrate`. In `ask` mode the tool requests user confirmation; in `allowed` mode integration is pre-authorized.
